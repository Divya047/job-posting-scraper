...
postingsRouter.get("/", verifyToken, async (req: Request, res: Response) => {
  try {
      ...
      companies.rows.map(async (company: Company) => {
        try {
          ...
          const newPosts = posts.filter(
            (post) =>
              !oldPosts.includes(post) && post.trim() !== "" && post !== ""
          );
    ...
    const companyUrls: Record<string, string> = companies.rows.reduce(
      (acc, company: Company) => {
    ...
    Object.entries(newPostings).forEach(([company, jobs]) => {
      newPostings[company] = [...jobs, companyUrls[company]];
    });
    ...
    if (companyQuery.rowCount === 0) {
      ...
      try {
        ...
        if (!posts || posts.length === 0) {
          res
            .status(404)
            .json({ error: "No postings found for the given company" });
        }
